...
|---|------|------|------|
| 19 | `Ruby Embedded Code in Interpolation` | 293-302 | ~~both scopes covered: `meta.embedded.line.ruby` by "Ruby - Interpolated Code", `source.ruby.embedded.source` prefix-matched by "Ruby - String Interpolation" (`source.ruby.embedded`)~~ **Correction found during execution: this reasoning was wrong for the second scope.** `meta.embedded.line.ruby` is genuinely a safe delete (exact-string duplicate of "Ruby - Interpolated Code"). But `source.ruby.embedded.source` is a DIFFERENT, more-specific string than `source.ruby.embedded` — not the same scope, just a descendant of it — and `check-theme.mjs`'s conflict detector only compares exact-string duplicates, so it never flagged this. A more-specific selector generally outranks a less-specific one in TextMate matching regardless of array order, so deleting the exact-match rule left only the less-specific `source.ruby.embedded` rule (`rosaMexicano`, pink) to match tokens that were rendering `textPrimary` (off-white) before — a real, undocumented color regression the task review caught. **Fix:** don't delete `source.ruby.embedded.source` — carry it forward into syntax.js as its own explicit rule (`palette.textPrimary`), separate from `"Ruby - String Interpolation"`'s `source.ruby.embedded`. **General lesson for this table:** two scope strings being in an "ancestor/descendant" relationship (one a dot-segment prefix of the other) is NOT the same as being duplicates — only delete a generator.js rule for a scope that has an EXACT-STRING match elsewhere, never merely a prefix match. |
...
| JSX element tags (`entity.name.tag.jsx`) | deeper orange #fb8500 | hot pink #FF69B4 | normalization — sibling scopes `entity.name.tag.js`/`.tsx` were already hot pink; documented in docs/CONFIGURATION.md |
| Console methods (`console.log` etc.) | yellow #ffd60a bold | turquoise #06ffc8 bold | Task 5 Step 5 explicit fontStyle pass unified all four console-related scopes to one color |
